...
(function(EsriLeaflet){
  ...
  EsriLeaflet.Layers.BasemapLayer = L.TileLayer.extend({
    ...
    onAdd: function(map){
      ...
      if(map.options.maxZoom > this.options.maxZoom)
        map.options.maxZoom = this.options.maxZoom;
    },
    ...
    getAttribution:function(){
      if(!this.options.attribution)
       ...
       var attribution = '<span class="esri-attributions" style="line-height:14px; text-overflow:ellipsis;overflow:hidden;display:inline-block;cursor:pointer;">' + this.options.attribution + '</span>'/* + logo*/;
    ...
    _bindAttribution: function(){
      if(this._map && this._map.attributionControl) {
        var map = this._map.attributionControl._container;
        for(var i = 0; i < map.children.length; i++) {
          if(map.children[i].className == "esri-attributions") {
            this._map._elmAttribution = map.children[i];
            this._map._elmAttribution.style.width = this.options.attriWidth;
            this._map._elmAttribution.style.whiteSpace = "nowrap";
            L.DomEvent.on(this._map._elmAttribution,'click',this._eventAttribution,this);
            break;
          }
        }
      }
    },
    _eventAttribution: function(e){
      e.preventDefault();
      var elm = e.target;
      if(elm.style.whiteSpace == "nowrap") {
        ...
        elm.style.width = this.options.attriWidth;
